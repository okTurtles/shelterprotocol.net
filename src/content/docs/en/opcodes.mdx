...
<Aside type="caution">
  *This specification is beta quality. Anywhere there is a conflict between this specification and the first implementation of this specification ([Chelonia](https://github.com/okTurtles/libcheloniajs)) is a bug. Please [let us know](https://github.com/okTurtles/shelterprotocol.net/issues) if you come across any contradictions.*
</Aside>
...
[
  // key 1
  {
    ...
    "allowedActions": "*" | ["selector-1", "selector-2", ...], // OPTIONAL
    "permissionsContext": ["scope-1", "scope-2", ...], // OPTIONAL
...
`"purpose"` is an array of strings that state the key's purpose. Typically this will be either `"sig"` (for signing messages), or `"enc"` for encrypting/decrypting messages, or it could be both. `"aut"` indicates the key can be used to modifying the purpose of other keys with `OP_KEY_UPDATE`. Other arbitrary values are allowed too if they are useful to protocol implementers.
...
<Aside type="note">
  Shelter Protocol currently does not specify what these key types are, leaving it up to individual implementations to decide. Future protocol upgrades could standardize type names. For now, we encourage implementers to choose unique type names that are long and descriptive of the ciphers used.
</Aside>

`"permissions"` is a list of opcode values (e.g. `"ka"`, `"ae"`, etc.) that this key is allowed to perform. Alternatively, the special value `"*"` may be used to indicate all permissions are granted (to support extending the protocol with new opcodes). Important to note that a key with `"ka"` permissions cannot add a key with more permissions than it has.

In addition to opcode values, there are special permissions:

- `ae#inner`: Indicates this key has permission to perform an inner signature for `ae` messages. Keys with this permission MUST specify `"allowedActions"`.
- `au#inner`: Indicates this key has permission to perform an inner signature for `au` messages. Keys with this permission MUST specify `"allowedActions"`.
- `kd#self`: Allows keys with `ka` permission to delete keys they've created without granting full `kd` permissions.
...
`"allowedActions"` this field is optional **but mandatory** on all keys with either `"ae"` or `"au"` permissions. `"*"` means all actions are allowed, and an array specifies the specific actions allowed. Selectors can have a `#inner` suffix which means the key is allowed to perform this action only if it has an inner signature.

`"permissionsContext"` allows permissions to be further narrowed to various contexts (contexts are just strings of up to 255 characters in length). If specified, this key can only act on other keys that exist in one of these contexts. This is particularly useful for restricting `OP_KEY_UPDATE` and `OP_KEY_DEL` operations issued by this key.
...
<Aside type="note">
  A key that has `OP_KEY_UPDATE` permissions can be used to expand or restrict the permissions of another key, subject to the following conditions:

  1. It has a `ringLevel` equal to or less than the other key it is operating on.
  2. It cannot grant permissions it doesn't have.
  3. It cannot expand the purpose of another key if it doesn’t have that purpose itself, except for keys having the `aut` purpose, which are not subject to this restriction.
</Aside>

<Aside type="caution">
  Gotchas when mirroring `foreignKey` updates:
...
<Aside>
  When mirroring `foreignKey` updates, it is possible to see the same `OP_KEY_DEL` twice because two clients might broadcast it simultaneously. If this happens, the key being deleted won't exist anymore, and implementations should ignore the duplicate message.
</Aside>
